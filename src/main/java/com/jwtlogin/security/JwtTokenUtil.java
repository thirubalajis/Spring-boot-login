...
import com.jwtlogin.security.event.InvalidTokenRequestException;
...
public class JwtTokenUtil {
	...
	public void setSigningKey(String sk) {
		this.jwtTokenSecret=sk;
	}
	...
	public boolean validateJwtToken(String token) {
		try {
				...
				validateTokenNotForLoggedOutUsers(token);
				return true;
		}
		...
		}catch(InvalidTokenRequestException exp) {
	...
	private void validateTokenNotForLoggedOutUsers(String token) {
        OnUserLogoutSuccessEvent previouslyLoggedOutEvent = 
        		loggedOutJwtTokenCache.getLogoutEventForToken(token);
        if (previouslyLoggedOutEvent != null) {
        	Date logoutEventDate = previouslyLoggedOutEvent.getEventTime();
            String errorMessage = String.format("Token corresponds to an already logged out user [%s]. Please login again", logoutEventDate);
